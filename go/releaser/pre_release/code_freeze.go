...
func CodeFreeze(state *releaser.State) (*logging.ProgressLogging, func() string) {
	...
	return pl, func() string {
		...
		codeFreezePRName := fmt.Sprintf("[%s] Code Freeze for `v%s`", state.VitessRelease.ReleaseBranch, releaser.AddRCToReleaseTitle(state.VitessRelease.Release, state.Issue.RC))
		...
		pr := github.PR{
			...
			Body:   fmt.Sprintf("This Pull Request freezes the branch `%s` for `v%s`", state.VitessRelease.ReleaseBranch, releaser.AddRCToReleaseTitle(state.VitessRelease.Release, state.Issue.RC)),
